...
**Q: transpose 改的是 stride 不是内存排布，啥意思？**
> 一个 `[B,T,C]` 张量在物理内存里就是 B×T×C 个数挨着排。`transpose(1,2)` 后逻辑形状变成 `[B,C,T]`，但物理内存没动——PyTorch 只改了 stride（每一步跨几个元素），不拷贝数据。`view()` 要求物理内存连续才能重新"解读"形状；如果之前 transpose 过，内存不连续了，view 就报错，需要先 `.contiguous()` 拷贝一份。

**Q: stride 和 view 有啥区别？**
> stride 是张量的属性（每一维跨几个元素），view 是方法（改变张量形状）。都不改内存，只改"解读方式"。区别：view 要求连续内存，transpose 改 stride 不要求。
...
> 三件事：① $e^x>0$ → 任意实数变正数；② 所有输出加起来=1 → 变成概率分布；③ 大的值被 $e^x$ 放大 → 小的被压得更小 → "只关注大的，忽略小的"。

**Q: nh 个头是啥？啥是"头"？**
> 一个"头"= 一套独立的 Q/K/V 投影。nh=6 时，384 维切成 6 个 64 维子空间，各自独立做 Attention。6 个头可能分别关注"动词-宾语""形容词-名词""位置邻近"等不同模式。最后所有头拼回来。

**Q: 加权融合那步是干啥？**
> 让每个 token 的表示融入上下文。"sat" 本身只是个词向量，做完 Attention 后变成 12%×V_The + 65%×V_cat + 23%×V_sat——"sat 的新表示里融入了 cat 的语义信息"。不加这步，每个词只知道自己，不知道上下文。

**Q: 两个 embedding 直接逐元素相加，原来的信息还存在吗？**
> 不是"抹掉"，是"融合"。比如 token_emb("cat")=[0.1,0.2,-0.3] 加 pos_emb(3)=[0.0,-0.1,0.2] 得 [0.1,0.1,-0.1]——分不开了，但 Transformer 不在乎分开，它只在乎后续线性变换能从相加结果里提取有用模式。实验证明模型自动学会区分语义和位置信息。不拼接是因为拼接会让维度翻倍→后续所有层参数翻倍。
...
**Q: c_proj 维度是 (768,768)，看起来没变化，为什么还要这步？**
> 各头独立计算的结果需要跨头混合——c_proj 的权重矩阵提供了这种信息重组能力。打个比方：6 个头各自写了 6 份报告，c_proj 是主编，把 6 份报告整合成一篇。

**Q: 训练时随机切断一些注意力连接是啥意思？**
> Attention Dropout。训练时随机把 attention weights 的一些位置置零。效果是模型不能依赖某一条固定的注意力路径，必须学会多条路都 work → 更鲁棒。推理时不关（model.eval() 自动关闭 dropout）。
...
**Q: FP16 是啥？**
> 16 位浮点数（2 字节），FP32 占 4 字节。好处：省显存、算得快。代价：精度低，太小或太大的数会溢出。训练时用 AMP（自动混合精度）——大部分用 FP16 算，关键地方用 FP32 保精度。

**Q: 推理模式(model.eval)和训练模式(model.train)有啥区别？**
> | | train() | eval() |
> |------|:--:|:--:|
> | Dropout | 生效 | 关闭 |
...
**Q: loss 和 logits 分别是啥？**
> logits = 模型最后一层输出的原始分数（未经 softmax），shape=(B,T,V)；loss = 拿 logits 和正确答案(targets)算交叉熵得到的一个数，越小越好。
...
> 2026-06-11 精读中提出的 24 个问题，全部已融入各章节。

| # | 问题 | 答案位置 |
|:--:|------|------|
| 1 | 离散整数→连续向量无法理解 | §0.2 Token Embedding vs Position Embedding |
| 2 | embedding 相加后信息还存在吗 | §2.5 常见疑问 |
| 3 | wte、wpe 参数说明 | §7.1 组件清单 + 附录 C 参数速查表 |
| 4 | shape 是啥 | §0.4 流水线（每个步骤都标了 shape） |
| 5 | 函数说明附录 | 附录 D 源码函数速查 |
| 6 | 问题应在源码下面说明 | 全书结构：按源码顺序，每行注释紧跟代码 |
| 7 | softmax 数学原理 | §2.2 数学公式 + §2.5 常见疑问 |
| 8 | 加权融合为啥要这步 | §2.1 检索演示 + §2.5 常见疑问 |
| 9 | nh 个头是啥 | §2.5 常见疑问 + §4.2 forward |
| 10 | 源码注释应有步骤 | §4.2 forward（10 步 + shape 速查表） |
| 11 | 代码每一步要做什么 | §4.3 10 步 Shape 变化速查表 |
| 12 | C 维度和 T 维度 | §0.2 表格 + §0.4 流水线 |
| 13 | FP16 是啥 | §7.5 常见疑问 |
| 14 | 随机切断注意力连接 | §4.6 常见疑问 + 附录 A |
| 15 | transpose/view/stride/contiguous | §1.5 矩阵三件套 + §1.7 常见疑问 |
| 16 | 信息重组能力怎么表现 | §4.6 常见疑问（c_proj） |
| 17 | 先膨胀再压缩 | §5 MLP |
| 18 | stride 和 view 区别 | §1.7 常见疑问 |
| 19 | 残差是啥 | §6 Block |
| 20 | dropout 三种类型 | 附录 A |
| 21 | loss 和 logits | §7.3 forward + §7.5 常见疑问 |
| 22 | 推理和训练模式区别 | §7.5 常见疑问 |
| 23 | Temperature 缩放原理 | §7.4 generate 步骤④ |
| 24 | weight decay | §9 configure_optimizers |
